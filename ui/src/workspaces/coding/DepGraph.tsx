...
function readTokens() {
  ...
  return {
    accent:  s.getPropertyValue('--accent-primary').trim(),   // local nodes
    warning: s.getPropertyValue('--accent-warning').trim(),   // third-party
    fog:     s.getPropertyValue('--text-secondary').trim(),   // builtin (inactive)
    border:  s.getPropertyValue('--border-color').trim(),     // edge stroke
  }
}
...
export default function DepGraph({ nodes, edges }: { nodes: DepNode[]; edges: DepEdge[] }) {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      const { accent, warning, fog, border } = readTokens()
      ...
      nodeG
        ...
        .attr('fill', (d) => nodeColors[d.type] ?? fog)
        .attr('fill-opacity', 0.12)
        .attr('stroke', (d) => nodeColors[d.type] ?? fog)
        .attr('stroke-width', 1)
    ...
    // Re-render on container resize so the graph fills the panel correctly
    const ro = new ResizeObserver(() => {
      sim?.stop()
      sim = render() ?? undefined
    })
